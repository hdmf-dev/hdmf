...
from ..utils import AllowPositional, docval, get_docval, getargs, popargs
...
class AlignedDynamicTable(DynamicTable):
        ...
        *get_docval(DynamicTable.__init__),
        {
            ...
            "type": list,
            "doc": (
                "List of DynamicTables to be added to the container. NOTE: Only regular"
                " DynamicTables are allowed. Using AlignedDynamicTable as a category"
                " for AlignedDynamicTable is currently not supported."
            ),
            "default": None,
        },
        {
            "name": "categories",
            "type": "array_data",
            "doc": "List of names with the ordering of category tables",
            "default": None,
        },
    ...
    def __init__(self, **kwargs):  # noqa: C901
        ...
        if in_categories is not None:
            if len(in_categories) != len(in_category_tables):
                raise ValueError(
                    "%s category_tables given but %s categories specified"
        ...
        if in_category_tables is not None:
            ...
            for table_name, tabel_index in lookup_index.items():
                ...
                if tabel_index < 0:  # pragma: no cover
                    raise ValueError(
                        "DynamicTable %s listed in categories but does not appear in category_tables" % table_name
                    )  # pragma: no cover
                ...
                if len(category) != len(self):
                    raise ValueError(
                        "Category DynamicTable %s does not align, it has %i rows expected %i"
    ...
    def add_category(self, **kwargs):
        ...
        if len(category) != len(self):
            raise ValueError(
                "New category DynamicTable does not align, it has %i rows expected %i" % (len(category), len(self))
            )
        ...
        if isinstance(category, AlignedDynamicTable):
            raise ValueError(
                "Category is an AlignedDynamicTable. Nesting of AlignedDynamicTable is currently not supported."
            )
    ...
    @docval(
        {
            "name": "include_category_tables",
            "type": bool,
            "doc": "Ignore sub-category tables and just look at the main table",
            "default": False,
        },
        {
            "name": "ignore_category_ids",
            "type": bool,
            "doc": "Ignore id columns of sub-category tables",
            "default": False,
        },
    )
    ...
    def to_dataframe(self, **kwargs):
        ...
        dfs = [
            super().to_dataframe().reset_index(),
        ]
        ...
        names = [
            self.name,
        ] + list(self.category_tables.keys())
    ...
    def get(self, item, **kwargs):
        ...
        * **tuple**: Get a column, row, or cell from a particular category table. The tuple is expected to
        ...
        if isinstance(item, (int, list, np.ndarray, slice)):
            ...
            dfs = [
                super().get(item, **kwargs).reset_index(),
            ...
            if len(item) == 2:
                ...
                raise ValueError(
                    "Expected tuple of length 2 of the form [category, column], [row,"
                    " category], [row, (category, column)] or a tuple of length 3 of"
                    " the form [category, column, row], [row, category, column]"
                )

    @docval(
        {
            "name": "ignore_category_tables",
            "type": bool,
            "doc": "Ignore the category tables and only check in the main table columns",
            "default": False,
        },
        allow_extra=False,
    )
    ...
    @docval(
        *get_docval(DynamicTable.get_linked_tables),
        {
            "name": "ignore_category_tables",
            "type": bool,
            "doc": "Ignore the category tables and only check in the main table columns",
