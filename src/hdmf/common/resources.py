...
class ExternalResources(Container):
    ...
    def assert_external_resources_equal(left, right, check_dtype=True):
        ...
        try:
            pd.testing.assert_frame_equal(
                left.keys.to_dataframe(), right.keys.to_dataframe(), check_dtype=check_dtype
            )  # fmt: skip
        except AssertionError as e:
            errors.append(e)
        try:
            pd.testing.assert_frame_equal(
                left.files.to_dataframe(), right.files.to_dataframe(), check_dtype=check_dtype
            )
        except AssertionError as e:
            errors.append(e)
        try:
            pd.testing.assert_frame_equal(
                left.objects.to_dataframe(), right.objects.to_dataframe(), check_dtype=check_dtype
            )
        except AssertionError as e:
            errors.append(e)
        try:
            pd.testing.assert_frame_equal(
                left.entities.to_dataframe(), right.entities.to_dataframe(), check_dtype=check_dtype
            )
        except AssertionError as e:
            errors.append(e)
        try:
            pd.testing.assert_frame_equal(
                left.object_keys.to_dataframe(), right.object_keys.to_dataframe(), check_dtype=check_dtype
            )
        except AssertionError as e:
            errors.append(e)
        if len(errors) > 0:
    ...
    @docval(
        {"name": "key", "type": (str, Key), "doc": "The key to associate the entity with."},
    ...
    @docval(
        ...
        },
        {
            ...
            "default": None,
            "doc": (
                "The type of the object. This is also the parent in relative_path. If omitted, "
    ...
    def _add_object(self, **kwargs):
        ...
        """
        files_idx, container, object_type, relative_path, field = popargs(
            "files_idx", "container", "object_type", "relative_path", "field", kwargs
        )
    ...
    def get_key(self, **kwargs):
        ...
        if container is not None:
            ...
            object_field = self._check_object_field(
                file=file, container=container, relative_path=relative_path, field=field
            )
    ...
    def add_ref(self, **kwargs):
        ...
        if attribute is None:  # Trivial Case
            ...
            if isinstance(attribute_object, AbstractContainer):
                relative_path = ""
                object_field = self._check_object_field(
                    file=file, container=attribute_object, relative_path=relative_path, field=field
                )
                ...
                if parent_spec.data_type is None:
                    ...
                    if isinstance(container, parent_cls):
                        ...
                        object_field = self._check_object_field(
                            file=file, container=parent, relative_path=relative_path, field=field
                        )
                    else:
    ...
    @docval(
        ...
        },
        {
            ...
            "default": False,
            "doc": (
                "The bool to return a dataframe with all instances of the object_type.",
    ...
    def get_object_type(self, **kwargs):
        ...
        if all_instances:
            ...
            df = df.loc[
                (df["object_type"] == object_type) & (df["relative_path"] == relative_path) & (df["field"] == field)
            ]
        return df
    ...
    def get_object_entities(self, **kwargs):
        ...
        if attribute is None:
            object_field = self._check_object_field(
                file=file, container=container, relative_path=relative_path, field=field, create=False
            )
        else:
            object_field = self._check_object_field(
                file=file, container=container[attribute], relative_path=relative_path, field=field, create=False
            )
    ...
    def to_dataframe(self, **kwargs):
        ...
        entities_df = pd.concat(objs=[entities_df, keys_mapped_df], axis=1, verify_integrity=False)
        ...
        object_keys_df = pd.concat(objs=[object_keys_df, objects_mapped_df], axis=1, verify_integrity=False)
        files_df = self.files.to_dataframe().iloc[object_keys_df["files_idx"]].reset_index(drop=True)
        file_object_object_key_df = pd.concat(objs=[object_keys_df, files_df], axis=1, verify_integrity=False)
        ...
        result_df = pd.concat(
            ...
            objs=[
                pd.merge(
                    ...
                    file_object_object_key_df.iloc[
                        [
                            i,
                        ]
                    ],
    ...
    def from_norm_tsv(cls, **kwargs):
        ...
        er = ExternalResources(files=files, keys=keys, entities=entities, objects=objects, object_keys=object_keys)
        return er
