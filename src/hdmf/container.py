...
def _exp_warn_msg(cls):
    ...
    if isinstance(cls, type):
        pfx = cls.__name__
    msg = (
        "%s is experimental -- it may be removed in the future and is not guaranteed to maintain backward compatibility"
        % pfx
    )
    return msg
        ...
        for field_conf in all_fields_conf:
            ...
            if not hasattr(cls, pname):
                setattr(
                    cls,
                    pname,
                    property(cls._getter(field_conf), cls._setter(field_conf)),
                )
    ...
    @docval(
        {
            "name": "recurse",
            "type": bool,
            "doc": "whether or not to change the object ID of this container's children",
            "default": True,
        }
    )
    ...
    def parent(self, parent_container):
        ...
        if self.parent is not None:
            if isinstance(self.parent, AbstractContainer):
                raise ValueError(
                    "Cannot reassign parent to Container: %s. Parent is already: %s." % (repr(self), repr(self.parent))
                )
            else:
    ...
    @docval(
        {
            "name": "dataio",
            "type": DataIO,
            "doc": "the DataIO to apply to the data held by this Data",
        }
    )
    ...
    * 'create' to name the method for creating Container instances (only if a single type is specified)
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if not hasattr(cls, "__clsconf__"):
            raise TypeError(
                "MultiContainerInterface subclass %s is missing __clsconf__ attribute."
            ...
            args.append(
                {
                    "name": attr_name,
                    "type": (list, tuple, dict, container_type),
                    "doc": "%s to store in this interface" % cls.__join(container_type),
                    "default": dict(),
                }
            )
    ...
    def __build_class(cls, name, bases, classdict):
        ...
        if isinstance(cls.__clsconf__, dict):
            ...
            raise TypeError(
                "'__clsconf__' for MultiContainerInterface subclass %s must be a dict or a list of dicts."
                % cls.__name__
            )
    ...
    def __build_conf_methods(cls, conf_dict, conf_index, multi):
        ...
        if create is not None:
            if isinstance(container_type, type):
                ...
                msg = (
                    "Cannot specify 'create' key in __clsconf__ for"
                    " MultiContainerInterface subclass %s when 'type' key is not a"
                    " single type"
                    % cls.__name__
                )
                if multi:
...
class Table(Data):
    ...
    def from_dataframe(cls, **kwargs):
        ...
        for index, row in df.iterrows():
            if use_index:
                data.append([row[colname] if colname != df.index.name else index for colname in cls_cols])
            else:
