...
class LocalOntology(Ontology):
    ...
    @docval(*get_docval(Ontology.__init__, 'version', 'ontology_name', 'ontology_uri'),
            {'name': 'ontology_entities', 'type': dict, 'doc': 'Dictionary of ontology terms with corresponding ID and uri as a tuple/list', 'default': None})
    def __init__(self, **kwargs):
        call_docval_func(super().__init__, kwargs)
        if kwargs['ontology_entities'] is None:
    ...
    def write_ontology_yaml(self, **kwargs):
        ...
        if ontology_dict is None:
            ontology_dict=self.ontology_entities
...
class WebAPIOntology(LocalOntology):
    ...
    """
    @docval(*get_docval(LocalOntology.__init__, 'version', 'ontology_name', 'ontology_uri', 'ontology_entities'),
    ...
    @docval(*get_docval(Ontology.get_ontology_entity, 'key'),
            {'name': 'add_entity', 'type': bool, 'doc': 'Boolean to add to local ontology dictionary', 'default': False})
    def get_ontology_entity(self, **kwargs):
        key = kwargs['key']
        add_entity = kwargs['add_entity']
        ...
        try:
            ...
            if not request.ok:
                ...
                if add_entity:
                    return self.add_ontology_entity(key, [entity_id, entity_uri])
                else:
                    return entity_id, entity_uri
        else:
            return entity_id, entity_uri
...
class EnsemblOntology(WebAPIOntology):
    ...
    @docval(*get_docval(WebAPIOntology.__init__, 'version', 'ontology_entities'),
