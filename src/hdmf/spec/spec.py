...
class DtypeHelper:
    ...
    valid_primary_dtypes = set(
        list(primary_dtype_synonyms.keys()) + [vi for v in primary_dtype_synonyms.values() for vi in v]
    )
    ...
    def check_dtype(dtype):
        ...
        if not isinstance(dtype, RefSpec) and dtype not in DtypeHelper.valid_primary_dtypes:
            raise ValueError(
                "dtype '%s' is not a valid primary data type. Allowed dtypes: %s"
...
_attr_args = [
    ...
    },
    {
        "name": "dtype",
        "type": (str, RefSpec),
        "doc": "The data type of this attribute",
    },
    {
        "name": "shape",
        "type": (list, tuple),
    ...
    },
    {
        "name": "required",
        "type": bool,
        "doc": 'whether or not this attribute is required. ignored when "value" is specified',
        "default": True,
    },
    {
        "name": "parent",
    ...
    },
    {
        "name": "value",
        "type": None,
        "doc": "a constant value for this attribute",
        "default": None,
    },
    {
        "name": "default_value",
        "type": None,
        "doc": "a default value for this attribute",
        "default": None,
    },
]
...
class BaseStorageSpec(Spec):
    ...
    def __init__(self, **kwargs):
        ...
        if quantity in (ONE_OR_MANY, ZERO_OR_MANY):
            if name is not None:
                raise ValueError(
                    "Cannot give specific name to something that can exist multiple times: name='%s', quantity='%s'"
                    % (name, quantity)
                )
    ...
    def __init__(self, **kwargs):
        ...
        if self.name is not None:
            ...
            if self.quantity not in valid_quant_vals:
                raise ValueError(
                    "quantity %s invalid for spec with fixed name. Valid values are: %s"
...
_group_args = [
    ...
    },
    {
        ...
        "type": str,
        "doc": (
            "the name of the Group that is written to the file. If this argument is"
            " omitted, users will be required to enter a ``name`` field when creating"
            " instances of this data type in the API. Another option is to specify"
            " ``default_name``, in which case this name will be used as the name of the"
            " Group if no other name is provided."
        ),
        "default": None,
    },
    {
    ...
    },
    {
        ...
        "type": (str, int),
        "doc": (
            "the allowable number of instance of this group in a certain location. See"
            " table of options `here"
            " <https://schema-language.readthedocs.io/en/latest/description.html#quantity>`_."
            " Note that if youspecify ``name``, ``quantity`` cannot be ``'*'``,"
            " ``'+'``, or an integer greater that 1, because you cannot have more than"
            " one group of the same name in the same parent group."
        ),
        "default": 1,
    },
    {
...
class GroupSpec(BaseStorageSpec):
    ...
    def resolve_spec(self, **kwargs):
        ...
        for dt_spec in data_types:
            ...
            if existing_dt_spec is None or (
                (isinstance(existing_dt_spec, list) or existing_dt_spec.name is not None) and dt_spec.name is None
            ):
        ...
        for link_spec in target_types:
            ...
            if (
                existing_dt_spec is None
                or (isinstance(existing_dt_spec, list) or existing_dt_spec.name is not None)
                and link_spec.name is None
            ):
    ...
    @docval(
        {"name": "name", "type": str, "doc": "the name of the link"},
        raises="ValueError, if 'name' is not part of this spec",
    )
