...
class H5RoundTripMixin(metaclass=ABCMeta):
    ...
    def tearDown(self):
        ...
        if (os.path.exists(self.export_filename)
                and os.getenv("CLEAN_HDMF", '1') not in ('0', 'false', 'FALSE', 'False')):
    ...
    def test_roundtrip(self):
        ...
        self.check_read_container()

    def test_roundtrip_export(self):
        """
        Test whether the test Container read from an exported file has the same contents the original Container and
        validate the file.
        """
        self.read_container = self.roundtripExportContainer()
        self.check_read_container(ignore_hdmf_attrs=True)  # need to ignore the container source

    def check_read_container(self, ignore_hdmf_attrs=False):
    ...
    def roundtripExportContainer(self, cache_spec=False):
        """
        Write the Container to an HDF5 file, read the Container from the file, export it to another file,
        read the Container from that file, and return it.
        """
        ...
        self.export_reader = HDF5IO(self.export_filename, manager=self.manager, mode='r')
        try:
