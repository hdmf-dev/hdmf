...
6. Publish the release tag.
  ...
  .. important::
      ...
      source distribution to the `HDMF PyPI project page`_ via PyPI trusted publishing and creates
      a new `GitHub release`_ as the github-actions bot.
...
8. Once the builds are completed, check that the distributions are available on `HDMF PyPI project page`_ and that
   a new `GitHub release`_ was created. The workflow fills the release notes from this version's section of
   ``CHANGELOG.md``, so check that they match. If the section was empty or its heading did not match the tag,
   the workflow falls back to auto-generated notes, which should be replaced by hand.
