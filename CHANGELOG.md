...
- Fixed `ObjectMapper.convert_dtype` raising `TypeError` on a numpy variable-length string array (`np.dtypes.StringDType`, kind `T`) for a text spec, and inferring `str` instead of `utf8` for such an array when the spec has no dtype. These arrays are now treated as `utf8` and passed through uncoerced, like object arrays of strings. The HDF5 backend also writes such an array in an attribute as variable-length text, and the validator reports the dtype of a dataset holding one as `utf` instead of the dtype object itself. @h-mayorquin [#1576](https://github.com/hdmf-dev/hdmf/pull/1576)
