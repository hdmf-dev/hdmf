...
- Fixed `HDMFIO.export` writing a broken link, which fails only when the file is read, when the exported container holds a container from another file that still has its original parent. Export raises a `ValueError` that names the link and says to call `reset_parent()` on that container before adding it. @rly [#1583](https://github.com/hdmf-dev/hdmf/pull/1583)
