...
- Migrated `hdmf.data_utils` off `@docval` to type-hinted signatures with `@validated` (`GenericDataChunkIterator`, `DataChunkIterator`, `DataChunk`, `DataIO`, `ShapeValidatorResult`). Downstream code is unaffected: `get_docval` on these constructors returns equivalent specs synthesized from the type hints, so patterns like `@docval(*get_docval(DataIO.__init__), ...)` (used by `H5DataIO` and extensions) keep working, positional/keyword calling conventions are unchanged, and error messages keep the docval format. @bendichter [#TBD]
