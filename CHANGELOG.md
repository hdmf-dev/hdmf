...
### Fixed
- Fixed selecting an empty region from an in-memory `DynamicTableRegion` (e.g. `table["region"][i]` for a ragged region row that references no target rows, or an empty slice) raising `IndexError: arrays used as indices must be of integer (or boolean) type` when the target table's columns hold their data as numpy arrays. `np.unique` of an empty selection returns a `float64` array, which is not a valid index into a numpy array; the unique row indices are now cast to an integer dtype. The same selection already worked after a write/read cycle, where the indices come back from HDF5 already integer-typed. @h-mayorquin [#1549](https://github.com/hdmf-dev/hdmf/pull/1549)
