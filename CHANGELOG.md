...
- Fixed building datasets with `dtype: isodatetime` when the spec uses `data_type_inc` (e.g., `VectorData`-extending columns). `datetime`/`date` values are now converted to ISO-formatted ASCII bytes via a dedicated `_isoformat` dtype helper instead of failing with `"Expected unicode or ascii string"`. Extended `get_class` to accept `datetime`/`date` for scalar datetime attributes and datasets. @rly [#1313](https://github.com/hdmf-dev/hdmf/pull/1313)
