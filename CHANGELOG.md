...
### Changed
- Refactored `HERD` object resolution into focused helpers (`_resolve_object_target`, `_find_object`, `_find_or_add_object`), replacing a single helper whose return type (an `Object` row or a dict) depended on a flag. The object lookup now identifies an object by its file together with its object_id, relative_path, and field, so a shared object_id across files (e.g. a copied, modified file) resolves to the correct object instead of colliding. A container that is not in a file continues to raise its own distinct error, separate from a container that is in a file but has no entry in the object table. Also de-duplicated the repetitive table-comparison and `from_zip` parsing/validation blocks. @rly [#1515](https://github.com/hdmf-dev/hdmf/pull/1515)
