...
- End-to-end support for `isodatetime`/`datetime` dtype on datasets and attributes, including extension specs. Build, read, and roundtrip now work for `datetime`/`date` values, and `get_class` generates classes that accept them. Specifically: `Builder.matched_spec` is a new write-once slot recording the subspec each builder was matched to, set by both the read-path matcher in `ObjectMapper.__get_subspec_values` and the write-path sites in `ObjectMapper.__add_datasets`/`__add_groups`/`build`; the read path parses `isodatetime`/`datetime` values back into Python `datetime`/`date` objects via `__parse_datetime_on_read`; the write path's `_isoformat` converter handles `datetime`/`date` values in `convert_dtype`; the `scalar_data` docval macro now includes `datetime.datetime`/`datetime.date` so generated dtype-less classes (and the base `Data` class) accept scalar datetime values; `CustomClassGenerator.post_process` updates the inherited `data` arg's type/shape from the spec while preserving the parent's default; the `spec_ext` kwarg on `BuildManager.build`/`TypeMap.build`/`ObjectMapper.build` is renamed to `matched_spec` to match the slot. @rly [#1448](https://github.com/hdmf-dev/hdmf/issues/1448)
