...
class TestReadLink(TestCase):
    ...
    def test_broken_link(self):
        ...
        os.remove(self.target_path)
        with self.assertWarnsWith(BrokenLinkWarning, 'Path to Group has been  altered/broken at /link_to_test_group'):
    ...
    def test_broken_linked_data(self):
        ...
        os.remove(self.target_path)
        with self.assertWarnsWith(BrokenLinkWarning, 'Path to Group has been  altered/broken at /link_to_test_dataset'):
            with HDF5IO(self.link_path, manager=get_foo_buildmanager(), mode='r') as read_io:
                bldr = read_io.read_builder()
                self.assertDictEqual(bldr.links, {})
