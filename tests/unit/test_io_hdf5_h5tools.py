...
class H5IOTest(TestCase):
    ...
    def test__chunked_iter_fill(self):
        ...
        data_opts = {
            ...
            "sparselist3": [
                ...
                None,
                None,
            ],  # note: cannot process None in ndarray
            "nanlist": [
                [
                    [1, 2, 3, np.nan, np.nan, 6],
                    [np.nan, np.nan, 3, 4, np.nan, np.nan],
                ],
                [
                    [10, 20, 30, 40, np.nan, np.nan],
        ...
        buffer_size_opts = [
            ...
            3,
            4,
        ]  # data is divisible by some of these, some not
                ...
                for buffer_size in buffer_size_opts:
                    with self.subTest(
                        ...
                        with warnings.catch_warnings(record=True):
                            ...
                            msg = (
                                "Iterating over an axis other than the first dimension"
                                " of list or tuple data involves converting the data"
                                " object to a numpy ndarray, which may incur a"
                                " computational cost."
                            )
    ...
    @unittest.skipIf(
        "lzf" not in h5py_filters.encode,
        "LZF compression not supported in this h5py library install",
    )
    def test_write_dataset_list_compress_lzf(self):
        warn_msg = (
            "lzf compression may not be available on all installations of HDF5. Use of"
            " gzip is recommended to ensure portability of the generated HDF5 files."
        )
    ...
    def test_write_dataset_list_enable_default_compress(self):
        a = H5DataIO(np.arange(30).reshape(5, 2, 3), compression=True)
    ...
    def test_write_dataset_list_disable_default_compress(self):
        msg = (
            "Compression disabled by compression=False setting. compression_opts parameter will, therefore, be ignored."
        )
    ...
    def test_write_dataset_iterable_multidimensional_array_compression(self):
        ...
        self.io.write_dataset(
            self.f,
            DatasetBuilder("test_dataset", wrapped_daiter, attributes={}),
        )
        dset = self.f["test_dataset"]
    ...
    def test_write_dataset_data_chunk_iterator(self):
        ...
        self.io.write_dataset(
            self.f,
            DatasetBuilder("test_dataset", dci, attributes={}, dtype=dci.dtype),
        )
        dset = self.f["test_dataset"]
        self.assertListEqual(dset[:].tolist(), list(range(10)))
    ...
    def test_error_on_unsupported_compression_filter(self):
        ...
        with self.assertRaises(ValueError):
            warn_msg = (
                "unknown compression may not be available on all installations of HDF5."
                " Use of gzip is recommended to ensure portability of the generated"
                " HDF5 files."
            )
            with self.assertWarnsWith(UserWarning, warn_msg):
        ...
        if not h5py_filters.h5z.filter_avail(h5py_filters.h5z.FILTER_MAX):
            with self.assertRaises(ValueError):
                warn_msg = (
                    "%i compression may not be available on all installations of HDF5."
                    " Use of gzip is recommended to ensure portability of the generated"
                    " HDF5 files."
                ...
                with self.assertWarnsWith(UserWarning, warn_msg):
                    H5DataIO(
                        np.arange(30),
                        compression=h5py_filters.h5z.FILTER_MAX,
                        allow_plugin_filters=True,
                    )
    ...
    def test_link_h5py_dataset_input(self):
        ...
        self.io.write_dataset(
            self.f,
            DatasetBuilder("test_softlink", self.f["test_dataset"], attributes={}),
        )
    ...
    def test_copy_h5py_dataset_input(self):
        ...
        self.assertListEqual(self.f["test_dataset"][:].tolist(), self.f["test_copy"][:].tolist())
    ...
    def test_link_h5py_dataset_h5dataio_input(self):
        ...
        self.io.write_dataset(
            ...
            DatasetBuilder(
                "test_softlink",
                H5DataIO(data=self.f["test_dataset"], link_data=True),
                attributes={},
            ),
        )
        self.assertTrue(isinstance(self.f.get("test_softlink", getlink=True), SoftLink))
    ...
    def test_copy_h5py_dataset_h5dataio_input(self):
        ...
        self.io.write_dataset(
            ...
            DatasetBuilder(
                "test_copy",
                H5DataIO(data=self.f["test_dataset"], link_data=False),  # Force dataset copy
                attributes={},
            ),
    ...
    def test_list_fill_empty(self):
        dset = self.io.__list_fill__(
            self.f,
            "empty_dataset",
            [],
            options={"dtype": int, "io_settings": {}},
        )
    ...
    def test_read_str(self):
        ...
        self.io.write_dataset(
            self.f,
            DatasetBuilder("test_dataset", a, attributes={"test_attr": attr}, dtype="text"),
        )
        self.io.close()
    ...
    def test_roundtrip_basic(self):
        ...
        with HDF5IO(self.path, manager=self.manager, mode="r") as io:
            ...
            self.assertListEqual(
                foofile.buckets["bucket1"].foos["foo1"].my_data,
...
class TestHDF5IO(TestCase):
    ...
    def test_delete_with_incomplete_construction_missing_file(self):
        ...
        with self.assertRaisesWith(
            exc_type=ValueError,
            exc_msg="interrupt before HDF5IO.__file is initialized",
        ):
            with MyHDF5IO() as _:
                pass
    ...
    def test_delete_with_incomplete_construction_missing_open_files(self):
        ...
        with self.assertRaisesWith(
            exc_type=ValueError,
            exc_msg="interrupt before HDF5IO.__open_files is initialized",
        ):
            with MyHDF5IO() as _:
                pass
...
class HDF5IOMultiFileTest(TestCase):
    ...
    def test_copy_file_with_external_links(self):
        ...
        foo2 = Foo(
            "foo2",
            read_foofile1.buckets["bucket1"].foos["foo1"].my_data,
            "I am foo2",
            34,
            6.28,
        )
...
class HDF5IOReadData(TestCase):
    ...
    def test_read_file_w(self):
        with HDF5IO(self.path, manager=get_foo_buildmanager(), mode="w") as io:
            with self.assertRaisesWith(
                UnsupportedOperation,
                "Cannot read from file %s in mode 'w'. Please use mode 'r', 'r+', or 'a'." % self.path,
            ):
                read_foofile1 = io.read()
                self.assertListEqual(
                    self.foofile1.buckets["bucket1"].foos["foo1"].my_data,
...
class HDF5IOWriteFileExists(TestCase):
    ...
    def test_write_w(self):
        ...
        with HDF5IO(self.path, manager=get_foo_buildmanager(), mode="r") as io:
            read_foofile = io.read()
            self.assertListEqual(
                self.foofile2.buckets["bucket2"].foos["foo2"].my_data,
    ...
    def test_write_r(self):
        with HDF5IO(self.path, manager=get_foo_buildmanager(), mode="r") as io:
            with self.assertRaisesWith(
                UnsupportedOperation,
                "Cannot write to file %s in mode 'r'. Please use mode 'r+', 'w', 'w-', 'x', or 'a'" % self.path,
            ):
                io.write(self.foofile2)
...
class H5DataIOValid(TestCase):
    ...
    def test_link(self):
        ...
        with HDF5IO(self.paths[0], manager=get_foo_buildmanager(), mode="r") as io:
            ...
            self.foo2 = Foo(
                "foo2",
                H5DataIO(data=read_foofile1.buckets["bucket1"].foos["foo1"].my_data),
                "I am foo2",
                17,
                3.14,
            )
        ...
        with self.assertRaisesWith(
            InvalidDataIOError,
            "Cannot get attribute 'shape' of data. Data is not valid.",
        ):
            self.foo2.my_data.shape

        with self.assertRaisesWith(
            InvalidDataIOError,
            "Cannot convert data to array. Data is not valid.",
        ):
...
class TestReadLink(TestCase):
    ...
    def test_broken_link(self):
        ...
        with self.assertWarnsWith(
            BrokenLinkWarning,
            "Path to Group altered/broken at /link_to_test_group",
        ):
    ...
    def test_broken_linked_data(self):
        ...
        with self.assertWarnsWith(
            BrokenLinkWarning,
            "Path to Group altered/broken at /link_to_test_dataset",
        ):
            with HDF5IO(self.link_path, manager=get_foo_buildmanager(), mode="r") as read_io:
                bldr = read_io.read_builder()
                self.assertDictEqual(bldr.links, {})
...
class TestLoadNamespaces(TestCase):
    ...
    def test_load_namespaces_none_version(self):
        ...
        with h5py.File(self.path, mode="r+") as f:
            # rename the group
            f.move(
                "/specifications/test_core/0.1.0",
                "/specifications/test_core/None",
            )

            # replace the namespace dataset with a serialized dict with the version key set to 'None'
            new_ns = (
                '{"namespaces":[{"doc":"a test'
                ' namespace","schema":[{"source":"test"}],"name":"test_core",'
                '"version":"None"}]}'
            )
    ...
    def test_load_namespaces_unversioned(self):
        ...
        with h5py.File(self.path, mode="r+") as f:
            ...
            f.move(
                "/specifications/test_core/0.1.0",
                "/specifications/test_core/unversioned",
            )
        ...
        ns_catalog = NamespaceCatalog()
        msg = (
            "Loaded namespace '%s' is missing the required key 'version'. Version will"
            " be set to '%s'. Please notify the extension author." % (CORE_NAMESPACE, SpecNamespace.UNVERSIONED)
        )
        with self.assertWarnsWith(UserWarning, msg):
            HDF5IO.load_namespaces(ns_catalog, self.path)
    ...
    def test_load_namespaces_resolve_custom_deps(self):
        ...
        with h5py.File(self.path, mode="r+") as f:
            ...
            added_types = (
                ',{"data_type_def":"BigFoo","data_type_inc":"Foo","doc":"doc","attributes":['
            ...
            new_ns = (
                '{"namespaces":[{"doc":"a test namespace","schema":['
            ...
            f.create_dataset(
                "/specifications/test-ext/0.1.0/test-ext.extensions",
                data=new_ext,
            )
...
class TestExport(TestCase):
    ...
    def test_basic(self):
        ...
        with HDF5IO(self.paths[1], manager=get_foo_buildmanager(), mode="r") as read_io:
            ...
            self.assertEqual(
                os.path.abspath(read_foofile.buckets["bucket1"].foos["foo1"].my_data.file.filename),
                self.paths[1],
            )
        ...
        with HDF5IO(self.paths[0], manager=get_foo_buildmanager(), mode="r") as read_io:
            ...
            with HDF5IO(self.paths[1], mode="w") as export_io:
                ...
                with self.assertRaisesWith(ValueError, msg):
                    export_io.export(
                        src_io=read_io,
                        container=read_foofile.buckets["bucket1"],
                    )
    ...
    def test_attr_reference(self):
        ...
        with HDF5IO(self.paths[1], manager=get_foo_buildmanager(), mode="r") as read_io:
            ...
            self.assertIs(
                read_foofile2.foo_ref_attr,
                read_foofile2.buckets["bucket1"].foos["foo1"],
            )
    ...
    def test_pop_linked_group(self):
        ...
        with HDF5IO(self.paths[0], manager=get_foo_buildmanager(), mode="r") as read_io:
            ...
            with HDF5IO(self.paths[1], mode="w") as export_io:
                msg = (
                    "links (links): Linked Foo 'foo1' has no parent. Remove the link or"
                    " ensure the linked container is added properly."
                )
    ...
    def test_append_data(self):
        ...
        with HDF5IO(self.paths[0], manager=get_foo_buildmanager(), mode="r") as read_io:
            ...
            foo2 = Foo(
                "foo2",
                read_foofile.buckets["bucket1"].foos["foo1"].my_data,
                "I am foo2",
                17,
                3.14,
            )
        ...
        with HDF5IO(self.paths[1], manager=get_foo_buildmanager(), mode="r") as read_io:
            ...
            self.assertIs(
                read_foofile2.foo_link,
                read_foofile2.buckets["bucket2"].foos["foo2"],
            )

            # test new soft link to new soft link to dataset in file
            self.assertIs(
                read_foofile2.buckets["bucket1"].foos["foo1"].my_data,
                read_foofile2.foofile_data,
            )
    ...
    def test_append_external_link_data(self):
        ...
        with HDF5IO(self.paths[0], manager=get_foo_buildmanager(), mode="r") as read_io1:
            ...
            with HDF5IO(self.paths[2], manager=get_foo_buildmanager(), mode="r") as read_io2:
                ...
                self.assertEqual(
                    read_foofile4.foofile_data,
                    read_foofile3.buckets["bucket1"].foos["foo1"].my_data,
                )

        with File(self.paths[2], "r") as f:
            self.assertEqual(
                f["buckets/bucket2/foo_holder/foo2/my_data"].file.filename,
                self.paths[0],
            )
            self.assertEqual(f["foofile_data"].file.filename, self.paths[0])
            self.assertIsInstance(
                f.get("buckets/bucket2/foo_holder/foo2/my_data", getlink=True),
                h5py.ExternalLink,
            )
    ...
    def test_export_simple_link_data(self):
        ...
        with File(self.paths[2], "r") as f:
            ...
            self.assertEqual(f["foofile_data"].file.filename, self.paths[0])
            self.assertIsInstance(f.get("buckets/bucket2/foo_holder/foo2/my_data", getlink=True), h5py.ExternalLink)
            self.assertIsInstance(f.get("foofile_data", getlink=True), h5py.ExternalLink)
    ...
    def test_export_cpd_dset_refs(self):
        ...
        with HDF5IO(self.paths[1], manager=get_baz_buildmanager(), mode="r") as read_io:
            ...
            for i in range(num_bazs):
                ...
                self.assertIs(
                    read_bucket2.baz_cpd_data.data[i][1],
                    read_bucket2.bazs[baz_name],
                )
    ...
    def test_roundtrip(self):
        ...
        with HDF5IO(self.path, manager=get_baz_buildmanager(), mode="r") as read_io:
            ...
            for i in range(num_bazs):
                ...
                self.assertIs(
                    read_bucket.baz_cpd_data.data[i][1],
                    read_bucket.bazs[baz_name],
                )
...
class TestWriteHDF5withZarrInput(TestCase):
    ...
    def test_roundtrip_basic(self):
        ...
        with HDF5IO(self.path, manager=self.manager, mode="r") as io:
            read_foofile = io.read()
            self.assertListEqual(
                foofile.buckets["bucket1"].foos["foo1"].my_data[:].tolist(),
                read_foofile.buckets["bucket1"].foos["foo1"].my_data[:].tolist(),
            )
    ...
    def test_write_zarr_int32_dataset(self):
        ...
        io.write_dataset(
            f,
            DatasetBuilder(name="test_dataset", data=zarr_data, attributes={}),
        )
        dset = f["test_dataset"]
...
class H5DataIOTests(TestCase):
    def _bad_arg_cm(self):
        return self.assertRaisesRegex(
            ValueError,
            "Must specify 'dtype' and 'shape' if not specifying 'data'",
        )
