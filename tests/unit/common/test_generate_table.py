...
class TestDynamicDynamicTable(TestCase):
    ...
    def test_dynamic_column(self):
        ...
        np.testing.assert_array_equal(test_table["indexed_col"].target.data, [1.0, 3.0, 2.0, 4.0])
        np.testing.assert_array_equal(test_table["dynamic_column"].data, [4, 4])

    def test_optional_col(self):
        test_table = self.TestTable(name="test_table", description="my test table")
        test_table.add_row(my_col=3.0, indexed_col=[1.0, 3.0], optional_col2=0.5)
        test_table.add_row(my_col=4.0, indexed_col=[2.0, 4.0], optional_col2=0.5)
    ...
    def test_dynamic_table_region(self):
        ...
        test_dtr_table = self.TestDTRTable(
            name="test_dtr_table",
