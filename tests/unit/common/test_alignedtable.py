...
class TestAlignedDynamicTableContainer(TestCase):
    ...
    * get_linked_tables
    ...
    def test_init(self):
        ...
        AlignedDynamicTable(name="test_aligned_table", description="Test aligned container")
    ...
    def test_init_duplicate_category_table_name(self):
        ...
        with self.assertRaisesWith(
            ValueError,
            "Duplicate table name test1 found in input dynamic_tables",
        ):
            categories = [
                DynamicTable(
                    name=val,
    ...
    def test_init_with_custom_nonempty_categories_and_main(self):
        ...
        with self.assertRaisesWith(
            ValueError,
            "Expected tuple of strings of length 2 got tuple of length 3",
        ):
            ("test1", "c1", "t3") in temp
    ...
    def test_get_item(self):
        ...
        with self.assertRaisesWith(
            ...
            (
                ...
                " category], [row, (category, column)] or a tuple of length 3 of the"
                " form [category, column, row], [row, category, column]"
            ),
        ):
            temp[("main_c1",)]
    ...
    def test_nested_aligned_dynamic_table_not_allowed(self):
        ...
        subsubcol1 = VectorData(
            name="sub_sub_column1",
            description="test sub sub column",
            data=["test11", "test12"],
        )
        sub_category = DynamicTable(
            name="sub_category1",
    ...
    def test_get_colnames(self):
        ...
        self.assertTupleEqual(
            adt.get_colnames(include_category_tables=False, ignore_category_ids=True),
            expected_colnames,
        )
        # Full set of columns
        expected_colnames = [
        ...
        self.assertListEqual(
            adt.get_colnames(include_category_tables=True, ignore_category_ids=False),
            expected_colnames,
        )
        ...
        self.assertListEqual(
            adt.get_colnames(include_category_tables=True, ignore_category_ids=True),
            expected_colnames,
        )
