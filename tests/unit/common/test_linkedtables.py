...
class TestLinkedAlignedDynamicTables(TestCase):
    ...
    def setUp(self):
        ...
        self.table_level0_0.add_column(
            data=["tag1", "tag2", "tag2", "tag1", "tag3", "tag4", "tag5"],
            name="tags",
        ...
        self.table_level0_1.add_column(
            data=["tag1", "tag1", "tag2", "tag2", "tag3", "tag3", "tag4"],
            name="tags",
            description="custom tags",
            index=[2, 4, 6, 7],
        )
        ...
        self.aligned_table = AlignedDynamicTable(
            ...
            columns=[
                VectorData(name="a1", description="a1", data=np.arange(3)),
            ],
            colnames=[
                "a1",
            ],
    ...
    def test_to_hierarchical_dataframe(self):
        ...
        self.assertListEqual(
            ...
            [
                ("my_aligned_table", "id"),
                ("my_aligned_table", ("my_aligned_table", "a1")),
                ("my_aligned_table", ("category0", "id")),
                ("my_aligned_table", ("category0", "filter")),
        ...
        self.assertListEqual(
            hier_df[("level0_0", "tags")].values.tolist(),
    ...
    def test_has_foreign_columns_false(self):
        ...
        temp_table = DynamicTable(
            name="t1",
            description="t1",
            colnames=["c1", "c2"],
            columns=[
                VectorData(name="c1", description="c1", data=np.arange(4)),
    ...
    def test_get_foreign_columns(self):
        ...
        for i, v in enumerate(
            [
                ("category0", "child_table_ref1"),
                ("category1", "child_table_ref1"),
            ]
        ):
    ...
    def test_get_foreign_columns_none(self):
        ...
        self.assertListEqual(
            temp_aligned_table.get_foreign_columns(ignore_category_tables=True),
            [],
        )

    def test_get_foreign_column_in_main_and_category_table(self):
        temp_table0 = DynamicTable(
            name="t0",
            description="t1",
            colnames=["c1", "c2"],
            columns=[
        ...
        self.assertListEqual(
            temp_aligned_table.get_foreign_columns(),
            [(None, "a2"), ("t1", "c2")],
        )
    ...
    def test_get_linked_tables(self):
        ...
        self.assertTupleEqual(
            (
                linked_tables[0].source_table.name,
        ...
        self.assertTupleEqual(
            (
                linked_tables[1].source_table.name,
    ...
    def test_get_linked_tables_none(self):
        ...
        self.assertListEqual(
            temp_aligned_table.get_linked_tables(ignore_category_tables=True),
            [],
        )
    ...
    def test_get_linked_tables_complex_link(self):
        ...
        for i, v in enumerate([("my_aligned_table", "a2", "t1"), ("t1", "c2", "t0")]):
            self.assertTupleEqual(
                (
                    linked_tables[i].source_table.name,
                    linked_tables[i].source_column.name,
                    linked_tables[i].target_table.name,
                ),
                v,
            )
...
class TestHierarchicalTable(TestCase):
    def setUp(self):
        ...
        self.aligned_table = AlignedDynamicTable(
            ...
            columns=[
                VectorData(name="a1", description="a1", data=np.arange(4)),
            ],
            colnames=[
                "a1",
            ],
        ...
        self.parent_table = DynamicTable(
            ...
            description="parent_table",
            columns=[
                VectorData(name="p1", description="p1", data=np.arange(4)),
                DynamicTableRegion(
                    name="l1",
                    description="l1",
                    data=np.arange(4),
        ...
        self.super_parent_table = DynamicTable(
            name="super_parent_table",
            description="super_parent_table",
            columns=[
                VectorData(name="sp1", description="sp1", data=np.arange(3)),
                dtr_sp,
                vi_dtr_sp,
            ],
        )
    ...
    def test_to_hierarchical_dataframe_no_dtr_on_top_level(self):
        ...
        spttable = DynamicTable(
            name="super_parent_table",
            description="super_parent_table",
            columns=[
                VectorData(name="sp1", description="sp1", data=np.arange(4)),
                dtr_sp,
            ],
        )
        hier_df = to_hierarchical_dataframe(spttable).reset_index()
        expected_columns = [
            ("super_parent_table", "id"),
            ("super_parent_table", "sp1"),
            ("parent_table", "id"),
            ("parent_table", "p1"),
            ("aligned_table", "id"),
    ...
    def test_to_hierarchical_dataframe_indexed_dtr_on_last_level(self):
        ...
        p1 = DynamicTable(
            ...
            description="parent_table",
            columns=[
                VectorData(name="p1", description="p1", data=np.arange(3)),
                dtr_p1,
                vi_dtr_p1,
            ],
        )
        ...
        self.assertListEqual(
            hier_df[("aligned_table", ("level0_0", "tags"))].to_list(),
            [["tag1"], ["tag2"], ["tag2", "tag1"]],
        )
    ...
    def test_to_hierarchical_dataframe_empty_tables(self):
        ...
        a1 = DynamicTable(
            name="level0_0",
            description="level0_0 DynamicTable",
        ...
        p1 = DynamicTable(
            ...
            description="parent_table",
            columns=[
                DynamicTableRegion(name="l1", description="l1", data=[], table=a1),
        ...
        spt = DynamicTable(
            ...
            columns=[
                dtr_sp,
                vi_dtr_sp,
                VectorData(name="sptc", description="l0", data=[]),
            ],
        )
    ...
    def test_to_hierarchical_dataframe_multilevel(self):
        ...
        self.assertListEqual(
            hier_df[("aligned_table", ("aligned_table", "a1"))].to_list(),
            list(range(3)),
        )
    ...
    def test_to_hierarchical_dataframe(self):
        ...
        self.assertListEqual(
            hier_df[("aligned_table", ("level0_0", "id"))].to_list(),
            list(range(10, 14)),
        )
        self.assertListEqual(
            hier_df[("aligned_table", ("level0_0", "myid"))].to_list(),
            list(range(4)),
        )
...
class TestLinkedDynamicTables(TestCase):
    ...
    def setUp(self):
        ...
        self.table_level0_0 = DynamicTable(name="level0_0", description="level0_0 DynamicTable")
        self.table_level0_1 = DynamicTable(name="level0_1", description="level0_1 DynamicTable")
        self.table_level1 = DynamicTableMultiDTR(
            name="level1",
    ...
    def popolate_tables(self):
        ...
        self.table_level1.add_column(
            data=["tag1", "tag2", "tag2", "tag3", "tag3", "tag4", "tag5"],
            name="tags",
            description="custom tags",
            index=[2, 4, 7],
        )
        # Level 2 data
        self.table_level2.add_row(
            id=0,
            child_table_ref1=[
                0,
            ],
    ...
    def test_get_foreign_columns(self):
        ...
        self.assertListEqual(
            self.table_level1.get_foreign_columns(),
