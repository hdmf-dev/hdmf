...
from hdmf.testing import H5RoundTripMixin, TestCase, remove_test_file
from hdmf.utils import StrDataset

from ..helpers.utils import get_temp_filepath
...
class TestDynamicTable(TestCase):
    ...
    def with_columns_and_data(self):
        columns = [
            VectorData(name=s["name"], description=s["description"], data=d) for s, d in zip(self.spec, self.data)
        ]
        return DynamicTable(
    ...
    def test_constructor_unequal_length_columns(self):
        columns = [
            VectorData(name="col1", description="desc", data=[1, 2, 3]),
    ...
    def test_add_column_vectorindex(self):
        ...
        msg = (
            "Passing a VectorIndex in for index may lead to unexpected behavior. This"
            " functionality will be deprecated in a future version of HDMF."
        )
    ...
    def test_add_column_multi_index(self):
        ...
        table.add_row(foo=5, bar=50.0, baz="lizard", qux=[[1, 2, 3], [1, 2, 3, 4]])
    ...
    def test_add_column_auto_index_int(self):
        ...
        table.add_column(name="qux", description="qux column", data=expected, index=1)
    ...
    def test_add_column_auto_index_bool(self):
        ...
        table.add_row(foo=5, bar=50.0, baz="lizard")
        expected = [[1, 2, 3], [1, 2, 3, 4]]
        table.add_column(name="qux", description="qux column", data=expected, index=True)
        self.assertListEqual(table["qux"][:], expected)
        self.assertListEqual(table.qux_index.data, [3, 7])
        # Add more rows after we created the column
        table.add_row(foo=5, bar=50.0, baz="lizard", qux=[10, 11, 12])
    ...
    def test_add_column_auto_multi_index_int(self):
        ...
        table.add_row(foo=5, bar=50.0, baz="lizard")
        table.add_row(foo=5, bar=50.0, baz="lizard")
        expected = [[[1, 2, 3], [1]], [[1, 2, 3, 4], [1, 2]]]
        table.add_column(name="qux", description="qux column", data=expected, index=2)
        self.assertListEqual(table["qux"][:], expected)
    ...
    def test_add_column_auto_multi_index_int_bad_index_levels(self):
        ...
        with self.assertRaisesWith(ValueError, msg):
            table.add_column(
                name="qux", description="qux column", data=expected, index=3
            )  # Too many index levels given
        # Asking for too few indexes will work here but should then later fail on write
        msg = (
            "Cannot automatically construct VectorIndex for nested array. Column data"
            " contains arrays as cell values. Please check the 'data' and 'index'"
            " parameters."
        )
        ...
        with self.assertRaisesWith(
            ValueError,
            msg + " 'index=True' may be too small for the given data.",
        ):
            table.add_column(name="qux", description="qux column", data=expected, index=True)
    ...
    def test_auto_multi_index_required(self):
        ...
        table.add_row(qux=[[[1, 2, 3], [1, 2, 3, 4]]])
        table.add_row(qux=[[[1, 2]]])

        expected = [[[[1, 2, 3], [1, 2, 3, 4]]], [[[1, 2]]]]
        self.assertListEqual(table["qux"][:], expected)
    ...
    def test_from_dataframe_eq(self):
        ...
        df = pd.DataFrame({"a": [[1, 2, 3], [1, 2, 3], [1, 2, 3]], "b": ["4", "5", "6"]})
        coldesc = {"a": "2d column", "b": "1d column"}
        received = DynamicTable.from_dataframe(
            df,
            "test_table",
    ...
    def test_from_dataframe_dup_attr(self):
        ...
        msg1 = (
            "An attribute 'parent' already exists on DynamicTable 'test' so this column"
            " cannot be accessed as an attribute, e.g., table.parent; it can only be"
            " accessed using other methods, e.g., table['parent']."
        )
...
"""
    ...
    def test_add_column_existing_attr(self):
        ...
        for attr in attrs:
            with self.subTest(attr=attr):
                msg = (
                    "An attribute '%s' already exists on DynamicTable"
                    " 'with_table_columns' so this column cannot be accessed as an"
                    " attribute, e.g., table.%s; it can only be accessed using other"
                    " methods, e.g., table['%s']." % (attr, attr, attr)
                )
    ...
    def test_init_columns_existing_attr(self):
        ...
        for attr in attrs:
            with self.subTest(attr=attr):
                ...
                msg = (
                    "An attribute '%s' already exists on DynamicTable 'test_table' so"
                    " this column cannot be accessed as an attribute, e.g., table.%s;"
                    " it can only be accessed using other methods, e.g., table['%s']." % (attr, attr, attr)
                )
                with self.assertWarnsWith(UserWarning, msg):
                    DynamicTable(
                        name="test_table",
    ...
    def test_multidim_col_one_elt_list(self):
        ...
        df = table.to_dataframe()
        df2 = pd.DataFrame({"data": [x for x in data]}, index=pd.Index(name="id", data=[0]))
        pd.testing.assert_frame_equal(df, df2)
...
class TestDynamicTableRoundTrip(H5RoundTripMixin, TestCase):
    def setUpContainer(self):
        ...
        table.add_row(
            ...
            bar=38.0,
            baz="dog",
            corge=[[11, 12, 13], [14, 15, 16]],
            qux=False,
            quux="b",
        )
        table.add_column(
            name="agv",
            description="a column with autogenerated multi vector index",
...
class DynamicTableRegionRoundTrip(H5RoundTripMixin, TestCase):
    def make_tables(self):
        ...
        target_columns = [
            VectorData(name=s["name"], description=s["description"], data=d) for s, d in zip(self.spec2, self.data2)
        ]
        ...
        columns = [
            VectorData(name=s["name"], description=s["description"], data=d) for s, d in zip(self.spec1, self.data1)
        ]
        columns.append(
            DynamicTableRegion(
                name="dtr",
                description="example DynamicTableRegion",
                data=[0, 1, 1, 0, 1],
                table=target_table,
            )
        )
    ...
    def _assert_two_elem_df(self, rec):
        ...
        data = [[1, 10.0, "cat", 0], [2, 20.0, "dog", 1]]
...
class SubTable(DynamicTable):
    __columns__ = (
        ...
        {
            "name": "col3",
            "description": "required, indexed column",
            "required": True,
            "index": True,
        },
        {
            "name": "col4",
            "description": "optional, indexed column",
            "index": True,
        },
        {
            "name": "col5",
        ...
        {
            "name": "col7",
            "description": "required, indexed region",
            "required": True,
            "index": True,
            "table": True,
        },
        {
            "name": "col8",
            "description": "optional, indexed region",
            "index": True,
            "table": True,
        },
        {
            "name": "col10",
            "description": "optional, indexed enum column",
            "index": True,
            "class": EnumData,
        },
        {
            "name": "col11",
            "description": "optional, enumerable column",
            "enum": True,
            "index": True,
        },
    )
...
class TestDynamicTableClassColumns(TestCase):
    ...
    def test_add_opt_column_mismatched_table_true(self):
        ...
        table = SubTable(name="subtable", description="subtable description")
        msg = (
            "Column 'col2' is predefined in SubTable with table=False which does not"
            " match the entered table argument. The predefined table spec will be"
            " ignored. Please ensure the new column complies with the spec. This will"
            " raise an error in a future version of HDMF."
        )
        with self.assertWarnsWith(UserWarning, msg):
    ...
    def test_add_opt_column_mismatched_index_true(self):
        ...
        table = SubTable(name="subtable", description="subtable description")
        msg = (
            "Column 'col2' is predefined in SubTable with index=False which does not"
            " match the entered index argument. The predefined index spec will be"
            " ignored. Please ensure the new column complies with the spec. This will"
            " raise an error in a future version of HDMF."
        )
        with self.assertWarnsWith(UserWarning, msg):
    ...
    def test_add_opt_column_mismatched_col_cls(self):
        ...
        table = SubTable(name="subtable", description="subtable description")
        msg = (
            "Column 'col10' is predefined in SubTable with class=<class"
            " 'hdmf.common.table.EnumData'> which does not match the entered col_cls"
            " argument. The predefined class spec will be ignored. Please ensure the"
            " new column complies with the spec. This will raise an error in a future"
            " version of HDMF."
        )
        with self.assertWarnsWith(UserWarning, msg):
...
class SelectionTestMixin:
    def setUp(self):
        ...
        self.table3 = DynamicTable(name="table3", description="a test table", id=[20, 21, 22])
        self.table3.add_column("foo", "scalar column", data=self._wrap([20.0, 21.0, 22.0]))
        self.table3.add_column(
            "bar",
            "ragged column",
            index=self._wrap([2, 3, 6]),
        ...
        self.table2 = DynamicTable(name="table2", description="a test table", id=[10, 11, 12])
        self.table2.add_column("foo", "scalar column", data=self._wrap([10.0, 11.0, 12.0]))
        self.table2.add_column(
            "bar",
            "ragged column",
            index=self._wrap([2, 3, 6]),
        ...
        self.table1 = DynamicTable(name="table1", description="a table to test slicing", id=[0, 1])
        self.table1.add_column("foo", "scalar column", data=self._wrap([0.0, 1.0]))
        self.table1.add_column(
            "bar",
            "ragged column",
            index=self._wrap([2, 3]),
            data=self._wrap(["r11", "r12", "r21"]),
        )
        self.table1.add_column(
            "baz",
            "multi-dimension column",
            data=self._wrap([[10.0, 11.0, 12.0], [20.0, 21.0, 22.0]]),
        )
        ...
        self.table1.add_column(
            "barz",
            "ragged column of tuples (cpd type)",
            index=self._wrap([2, 3]),
        ...
        data["bar"] = [
            self._wrap_check(["r11", "r12"]),
            self._wrap_check(["r21"]),
        ]
        data["baz"] = [
            self._wrap_check([10.0, 11.0, 12.0]),
            self._wrap_check([20.0, 21.0, 22.0]),
        ]
        data["barz"] = [
            self._wrap_check([(1.0, 11), (2.0, 12)]),
            self._wrap_check([(3.0, 21)]),
        ]
        idx = [0, 1]
...
class TestSelectionH5Dataset(SelectionTestMixin, TestCase):
    ...
    def _wrap(self, my_list):
        ...
        if isinstance(my_list[0], str):
            ...
            cpd_type = [
                dict(name="cpd_float", dtype=np.dtype("float64")),
...
class TestDoubleIndex(TestCase):
    ...
    def test_add_vector(self):
        ...
        self.assertListEqual(
            foo.data,
            ["a11", "a12", "a21", "b11", "c11", "c12", "c13", "c21", "c22"],
        )
...
class TestDTDoubleIndex(TestCase):
    def test_double_index(self):
        ...
        table = DynamicTable(
            ...
            columns=[foo, foo_ind, foo_ind_ind],
        )
    ...
    def test_double_index_reverse(self):
        ...
        table = DynamicTable(
            name="table0",
            description="an example table",
            columns=[foo_ind_ind, foo_ind, foo],
        )
    ...
    def test_double_index_colnames(self):
        ...
        table = DynamicTable(
            name="table0",
            description="an example table",
            columns=[foo, foo_ind, foo_ind_ind, bar],
            colnames=["foo", "bar"],
        )
    ...
    def test_double_index_reverse_colnames(self):
        ...
        table = DynamicTable(
            name="table0",
            description="an example table",
            columns=[foo_ind_ind, foo_ind, foo, bar],
            colnames=["bar", "foo"],
        )
...
class TestDynamicTableAddEnum(TestCase):
    def test_enum(self):
        ...
        exp = pd.DataFrame(
            data={"bar": ["a", "b", "a", "c"]},
            index=pd.Series(name="id", data=[0, 1, 2, 3]),
        )
...
class TestDataIOIndexedColumns(H5RoundTripMixin, TestCase):
    def setUpContainer(self):
        ...
        table = DynamicTable(
            name="table0",
            description="an example table",
            columns=[foo_ind, foo, bar_ind, bar],
        )

        # check for add_row
        table.add_row(
            foo=np.arange(30).reshape(5, 2, 3),
...
class TestDTRReferences(TestCase):
    ...
    def test_dtr_references(self):
        ...
        table1 = DynamicTable(name="table1", description="test table 1")
        table1.add_column(name="x", description="test column of ints")
        table1.add_column(name="y", description="test column of reference")
        table1.add_row(id=101, x=1, y=group1)
        table1.add_row(id=102, x=2, y=group1)
        table1.add_row(id=103, x=3, y=group2)

        table2 = DynamicTable(name="table2", description="test table 2")
        ...
        with HDF5IO(self.filename, manager=get_manager(), mode="r") as io:
            ...
            expected = pd.DataFrame(
                {"x": np.array([2, 3]), "y": [read_group1, read_group2]},
