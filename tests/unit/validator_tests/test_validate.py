...
from hdmf.validate.errors import (
    DtypeError,
    ExpectedArrayError,
    IllegalLinkError,
    ...
    def test_valid_isodatetime(self):
        builder = GroupBuilder(
            ...
            datasets=[
                ...
                DatasetBuilder("time", datetime(2017, 5, 1, 12, 0, 0, tzinfo=tzlocal())),
                DatasetBuilder(
                    "time_array",
...
class TestNestedTypes(ValidatorTestBase):
    def getSpecs(self):
        ...
        foo = GroupSpec(
            "A test group that contains a data type",
            data_type_def="Foo",
...
class TestQuantityValidation(TestCase):
    def create_test_specs(self, q_groups, q_datasets, q_links):
        ...
        foo = GroupSpec(
            "A group containing a quantity of tests and datasets",
            data_type_def="Foo",
    ...
    def get_test_builder(self, n_groups, n_datasets, n_links):
        ...
        child_links = [
            LinkBuilder(
                GroupBuilder(f"qux_{n}", attributes={"data_type": "Qux"}),
                f"qux_{n}_link",
            )
    ...
    def test_missing_one_or_many_should_not_return_incorrect_quantity_error(
        self,
    ):
    ...
    def test_missing_fixed_quantity_should_not_return_incorrect_quantity_error(
        self,
    ):
    ...
    def test_incorrect_fixed_quantity_should_return_incorrect_quantity_error(
        self,
    ):
    ...
    def test_incorrect_zero_or_one_quantity_should_return_incorrect_quantity_error(
        self,
    ):
...
class TestDtypeValidation(TestCase):
    ...
    def test_utf8_for_ascii(self):
        ...
        expected_errors = {
            "Bar/attr1 (my_bar.attr1): incorrect type - expected 'bytes', got 'utf'",
    ...
    def test_int8_for_int64(self):
        ...
        result_strings = set([str(s) for s in results])
        expected_errors = {
            "Bar/attr1 (my_bar.attr1): incorrect type - expected 'int64', got 'int8'",
    ...
    def test_bool_for_numeric(self):
        ...
        result_strings = set([str(s) for s in results])
        expected_errors = {
            "Bar/attr1 (my_bar.attr1): incorrect type - expected 'numeric', got 'bool'",
...
class Test1DArrayValidation(TestCase):
    ...
    def test_scalar(self):
        ...
        expected_errors = {
            (
                "Bar/attr1 (my_bar.attr1): incorrect shape - expected an array of shape"
                " '(None,)', got non-array data 'a string'"
            ),
            (
                "Bar/data (my_bar/data): incorrect shape - expected an array of shape"
                " '(None,)', got non-array data 'a string'"
            ),
        }
        self.assertEqual(result_strings, expected_errors)
...
class TestLinkable(TestCase):
    def set_up_spec(self):
        ...
        spec = GroupSpec(
            ...
            datasets=[
                DatasetSpec(
                    "A linkable child dataset",
                    name="untyped_linkable_ds",
                    linkable=True,
                    quantity=ZERO_OR_ONE,
                ),
                DatasetSpec(
                    "A non-linkable child dataset",
                    name="untyped_nonlinkable_ds",
                    linkable=False,
                    quantity=ZERO_OR_ONE,
                ),
                DatasetSpec(
            ...
            groups=[
                GroupSpec(
                    "A linkable child group",
                    name="untyped_linkable_group",
                    linkable=True,
                    quantity=ZERO_OR_ONE,
                ),
                GroupSpec(
                    "A non-linkable child group",
                    name="untyped_nonlinkable_group",
                    linkable=False,
                    quantity=ZERO_OR_ONE,
                ),
                GroupSpec(
...
class TestLinkAndChildMatchingDataType(TestCase):
    ...
    def test_error_on_missing_child_dataset(self):
        ...
        links = [
            ...
            LinkBuilder(
                name="group_link",
                builder=GroupBuilder("bar", attributes={"data_type": "Bar"}),
            ),
        ]
...
class TestMultipleChildrenAtDifferentLevelsOfInheritance(TestCase):
    ...
    def set_up_spec(self):
        spec_catalog = SpecCatalog()
        dataset_spec = DatasetSpec("A dataset", data_type_def="Foo")
        sub_dataset_spec = DatasetSpec("An Inheriting Dataset", data_type_def="Bar", data_type_inc="Foo")
        spec = GroupSpec(
            "A test group specification with a data type",
            data_type_def="Baz",
            datasets=[
...
class TestExtendedIncDataTypes(TestCase):
    ...
    def test_missing_additional_attribute_on_anonymous_data_type_extension(
        self,
    ):
    ...
    def getSpecs(self):
        ...
        foo_spec = GroupSpec(
            ...
            datasets=[
                DatasetSpec(
                    doc="optional Bar",
                    data_type_inc=bar_spec,
                    quantity=ZERO_OR_ONE,
                ),
                DatasetSpec(
                    doc="optional Baz",
                    data_type_inc=baz_spec,
                    quantity=ZERO_OR_ONE,
                ),
                DatasetSpec(
                    doc="multiple qux",
                    data_type_inc=qux_spec,
