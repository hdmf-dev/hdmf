...
class TestGetSubSpec(TestCase):
    ...
    def test_unnamed_link_data_type_inc(self):
        ...
        parent_spec = GroupSpec(doc="Something to hold a Bar", name="bar_bucket", links=[link_spec])
...
class TestObjectMapperNested(ObjectMapperMixin, TestCase):
    ...
    def test_build(self):
        ...
        expected = GroupBuilder(
            name="my_bar",
            datasets={"data": DatasetBuilder(name="data", data=list(range(10)), attributes={"attr2": 10})},
    ...
    def test_remap_keys(self):
        self._remap_nested_attr()
        self.assertEqual(
            self.mapper.get_attr_spec("attr2"),
            self.mapper.spec.get_dataset("data").get_attribute("attr2"),
        )
        self.assertEqual(
            self.mapper.get_attr_spec("attr1"),
            self.mapper.spec.get_attribute("attr1"),
        )
        self.assertEqual(
            self.mapper.get_attr_spec("data"),
            self.mapper.spec.get_dataset("data"),
        )
...
class TestLinkedContainer(TestCase):
    ...
    def test_build_child_link(self):
        ...
        inner_foo_builder = GroupBuilder(
            "my_foo",
            attributes={
                "data_type": "Foo",
                "namespace": CORE_NAMESPACE,
    ...
    def test_build_broken_link_no_parent(self):
        ...
        msg = (
            "my_bar2 (my_bar2): Linked Foo 'my_foo' has no parent. Remove the link or"
...
class TestReference(TestCase):
    ...
    def test_build_attr_ref(self):
        """Test default mapping functionality when one container contains an attr reference to another container."""
        foo_inst = Foo("my_foo")
        bar_inst1 = Bar("my_bar1", list(range(10)), "value1", 10, foo=foo_inst)
...
class TestFixedAttributeValue(ObjectMapperMixin, TestCase):
    def setUpBarSpec(self):
        self.bar_spec = GroupSpec(
            ...
            data_type_def="Bar",
            attributes=[
                AttributeSpec("attr1", "an example string attribute", "text", value="hi"),
                AttributeSpec("attr2", "an example integer attribute", "int"),
            ],
        )
    ...
    def test_required_attr_missing(self):
        ...
        expected = GroupBuilder(name="my_bar1", attributes={"attr1": "hi", "attr2": 10})
        self.assertBuilderEqual(builder, expected)
...
class TestObjectMapperBadValue(TestCase):
    def test_bad_value(self):
        ...
        class Qux(Container):
            @docval(
                {"name": "name", "type": str, "doc": "the name of this Qux"},
