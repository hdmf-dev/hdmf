...
from ..helpers.utils import CORE_NAMESPACE, Foo, FooBucket
...
class TestBuildManager(TestBase):
    def test_build(self):
        ...
        expected = GroupBuilder(
            "my_foo",
            datasets={"my_data": DatasetBuilder("my_data", list(range(10)), attributes={"attr2": 10})},
            attributes={
                "attr1": "value1",
    ...
    def test_clear_cache(self):
        ...
        builder = GroupBuilder(
            "my_foo",
            datasets={"my_data": DatasetBuilder("my_data", list(range(10)), attributes={"attr2": 10})},
            attributes={"attr1": "value1", "namespace": CORE_NAMESPACE, "data_type": "Foo", "object_id": -1},
        )
        container1 = self.manager.construct(builder)
        self.manager.clear_cache()
        container2 = self.manager.construct(builder)
...
class NestedBaseMixin(metaclass=ABCMeta):
    def setUp(self):
        ...
        self.foo_builders = {
            ...
            "my_foo2": GroupBuilder(
                "my_foo2",
                datasets={"my_data": DatasetBuilder("my_data", list(range(10, 20)), attributes={"attr2": 20})},
                attributes={
                    "attr1": "value2",
                    "namespace": CORE_NAMESPACE,
                    "data_type": "Foo",
    ...
    def setUpBucketBuilder(self):
        self.bucket_builder = GroupBuilder(
            ...
            attributes={
                "namespace": CORE_NAMESPACE,
                "data_type": "FooBucket",
                "object_id": self.foo_bucket.object_id,
            },
        )
...
class TestNestedContainersSubgroupSubgroup(NestedBaseMixin, TestBase):
    ...
    def setUpBucketSpec(self):
        ...
        tmp_spec = GroupSpec(
            "A subgroup to hold the subgroup",
            name="foo_holder_holder",
            groups=[tmp_spec],
        )
        self.bucket_spec = GroupSpec(
            "A test group specification for a data type containing data type",
    ...
    def setUpBucketMapper(self):
        class BucketMapper(ObjectMapper):
            def __init__(self, spec):
                ...
                self.map_spec(
                    "foos",
                    spec.get_group("foo_holder_holder").get_group("foo_holder").get_data_type("Foo"),
                )
...
class TestNoAttribute(TestBase):
    def test_build(self):
        ...
        container_inst = Foo("my_foo", list(range(10)), "value1", 10)
        msg = "Foo 'my_foo' does not have attribute 'unknown' for mapping to spec: %s" % self.foo_spec.get_dataset(
            "my_data"
        )
