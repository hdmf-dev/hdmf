...
from hdmf.spec.spec import DEF_QUANTITY, ONE_OR_MANY, ZERO_OR_MANY, ZERO_OR_ONE
from hdmf.testing import TestCase
from hdmf.utils import docval, getargs

from ...helpers.utils import CORE_NAMESPACE
...
class BasicBucket(Container):
    @docval(
        ...
        },
        {
            ...
            "type": "scalar_data",
            "doc": "a scalar dataset within this BasicBucket",
            "default": None,
        },
        {
            "name": "untyped_array_dataset",
            "type": ("data", "array_data"),
            "doc": "an array dataset within this BasicBucket",
            "default": None,
        },
    )
    def __init__(self, **kwargs):
...
class TypeIncUntypedGroupMixin:
    ...
    def setUpBucketMapper(self):
        class BucketMapper(ObjectMapper):
            def __init__(self, spec):
                ...
                self.map_spec(
                    "foos",
                    spec.get_group("foo_holder").get_data_type("SimpleFoo"),
                )
                self.unmap(spec.get_group("qux_holder"))
                self.map_spec(
                    "quxs",
                    spec.get_group("qux_holder").get_data_type("SimpleQux"),
                )
    ...
    def get_two_bucket_test(self):
        ...
        bucket = SimpleBucket(name="test_bucket", foos=foos, quxs=quxs, links=foos)
        ...
        foo_holder_builder = GroupBuilder(
            name="foo_holder",
            groups={"my_foo1": foo1_builder, "my_foo2": foo2_builder},
        )
        qux_holder_builder = GroupBuilder(
            name="qux_holder",
            datasets={"my_qux1": qux1_builder, "my_qux2": qux2_builder},
        )
        ...
        link_holder_builder = GroupBuilder(
            name="link_holder",
            links={"my_foo1": foo1_link_builder, "my_foo2": foo2_link_builder},
        )
    ...
    def get_one_bucket_test(self):
        ...
        foo_holder_builder = GroupBuilder(name="foo_holder", groups={"my_foo1": foo1_builder})
        ...
        qux_holder_builder = GroupBuilder(name="qux_holder", datasets={"my_qux1": qux1_builder})
        foo1_link_builder = LinkBuilder(builder=foo1_builder)
        link_holder_builder = GroupBuilder(name="link_holder", links={"my_foo1": foo1_link_builder})
        bucket_builder = GroupBuilder(
            name="test_bucket",
            groups={
                "foos": foo_holder_builder,
    ...
    def get_zero_bucket_test(self):
        ...
        foo_holder_builder = GroupBuilder(name="foo_holder", groups={})
        qux_holder_builder = GroupBuilder(name="qux_holder", datasets={})
        link_holder_builder = GroupBuilder(name="link_holder", links={})
        bucket_builder = GroupBuilder(
            name="test_bucket",
            groups={
                "foos": foo_holder_builder,
    ...
    def get_mismatch_bucket_test(self):
        ...
        quxs = [
            NotSimpleQux("my_qux1", data=[1, 2, 3]),
            NotSimpleQux("my_qux2", data=[4, 5, 6]),
        ]
        bucket = SimpleBucket(name="test_bucket", foos=foos, quxs=quxs, links=foos)
        foo_holder_builder = GroupBuilder(name="foo_holder", groups={})
        qux_holder_builder = GroupBuilder(name="qux_holder", datasets={})
...
class TypeIncMixin:
    ...
    def get_two_bucket_test(self):
        ...
        bucket = SimpleBucket(name="test_bucket", foos=foos, quxs=quxs, links=links)
        foo1_builder = self._create_builder(bucket.foos["my_foo1"])
        foo2_builder = self._create_builder(bucket.foos["my_foo2"])
        ...
        bucket_builder = GroupBuilder(
            ...
            groups={"my_foo1": foo1_builder, "my_foo2": foo2_builder},
            datasets={"my_qux1": qux1_builder, "my_qux2": qux2_builder},
            links={"my_foo3": foo3_link_builder, "my_foo4": foo4_link_builder},
            attributes={
                "namespace": CORE_NAMESPACE,
                "data_type": "SimpleBucket",
                "object_id": bucket.object_id,
...
class OneMixin:
    ...
    def test_build_two(self):
        """Test building a container which contains multiple containers as the spec allows."""
        bucket, bucket_builder = self.get_two_bucket_test()
        msg = r"SimpleBucket 'test_bucket' has 2 values for attribute '.*' but spec" r" allows 1\."
...
class TwoMixin:
    ...
    def test_build_one(self):
        """Test building a container which contains one container as the spec allows."""
        bucket, bucket_builder = self.get_one_bucket_test()
        msg = r"SimpleBucket 'test_bucket' has 1 values for attribute '.*' but spec" r" allows 2\."
        with self.assertWarnsRegex(IncorrectQuantityBuildWarning, msg):
            builder = self.manager.build(bucket)
        self.assertDictEqual(builder, bucket_builder)
...
class ZeroOrOneMixin:
    ...
    def test_build_two(self):
        """Test building a container which contains multiple containers as the spec allows."""
        bucket, bucket_builder = self.get_two_bucket_test()
        msg = r"SimpleBucket 'test_bucket' has 2 values for attribute '.*' but spec" r" allows '\?'\."
        with self.assertWarnsRegex(IncorrectQuantityBuildWarning, msg):
            builder = self.manager.build(bucket)
        self.assertDictEqual(builder, bucket_builder)
...
class TestBuildZeroOrManyTypeIncUntypedGroup(ZeroOrManyMixin, TypeIncUntypedGroupMixin, BuildQuantityMixin, TestCase):
    """Test a group that has an untyped subgroup with a data type inc subgroup/dataset/link with quantity '*'"""
...
class TestBuildOneTypeIncUntypedGroup(OneMixin, TypeIncUntypedGroupMixin, BuildQuantityMixin, TestCase):
    """Test a group that has an untyped subgroup with a data type inc subgroup/dataset/link with quantity 1"""

    pass


class TestBuildTwoTypeIncUntypedGroup(TwoMixin, TypeIncUntypedGroupMixin, BuildQuantityMixin, TestCase):
    """Test a group that has an untyped subgroup with a data type inc subgroup/dataset/link with quantity 2"""
...
class TestBuildOneUntyped(UntypedMixin, TestCase):
    ...
    def test_build_data(self):
        ...
        bucket_builder = GroupBuilder(
            ...
            datasets={
                "untyped_dataset": untyped_dataset_builder,
