...
When a builder's spec (or its resolved_spec, which captures position-specific
...
class TimestampedRecord(Container):
    ...
    @docval({'name': 'name', 'type': str, 'doc': 'name'},
            {'name': 'created_at', 'type': (datetime, date, str), 'doc': 'when',
             'default': None},
            {'name': 'tag', 'type': str, 'doc': 'a tag', 'default': 't'},
            {'name': 'stamped', 'type': StampedThing, 'doc': 'a stamped child', 'default': None})
...
class TestReadAttributeDatetime(TestCase):
    ...
    def test_attribute_isodatetime_with_time(self):
        spec = GroupSpec(
            doc='record', data_type_def='TimestampedRecord',
            attributes=[AttributeSpec('created_at', 'when', 'isodatetime'),
                        AttributeSpec('tag', 'a tag', 'text')],
        )
        gb = GroupBuilder('r',
                          attributes={'created_at': b'2024-03-15T12:30:00', 'tag': 't',
                                      'data_type': 'TimestampedRecord',
                                      'namespace': CORE_NAMESPACE,
                                      'object_id': 'rid'})
        record = _construct(spec, [], gb)
        self.assertEqual(record.created_at, datetime(2024, 3, 15, 12, 30))
    ...
    def test_attribute_isodatetime_date_only(self):
        ...
        gb = GroupBuilder('r',
                          attributes={'created_at': '2024-03-15', 'tag': 't',
                                      'data_type': 'TimestampedRecord',
                                      'namespace': CORE_NAMESPACE,
                                      'object_id': 'rid'})
        record = _construct(spec, [], gb)
    ...
    def test_attribute_already_parsed_passes_through(self):
        ...
        already = datetime(2024, 3, 15, 12, 30)
        gb = GroupBuilder('r',
                          attributes={'created_at': already, 'tag': 't',
                                      'data_type': 'TimestampedRecord',
                                      'namespace': CORE_NAMESPACE,
                                      'object_id': 'rid'})
        record = _construct(spec, [], gb)
...
class TestReadDatasetDatetime(TestCase):
    ...
    def test_def_site_isodatetime_array(self):
        ...
        child_def = DatasetSpec(doc='stamped', data_type_def='StampedThing', dtype='isodatetime',
                                shape=(None,))
        named_subspec = DatasetSpec(doc='stamped child', data_type_inc='StampedThing', name='stamped')
        parent_spec = GroupSpec(doc='record', data_type_def='TimestampedRecord',
                                attributes=[AttributeSpec('tag', 'a tag', 'text')],
                                datasets=[named_subspec])

        child_db = DatasetBuilder('stamped',
                                  np.array([b'2024-03-15T00:00:00', b'2024-04-01T12:00:00']),
                                  attributes={'data_type': 'StampedThing',
                                              'namespace': CORE_NAMESPACE,
                                              'object_id': 'sid'})
        gb = GroupBuilder('r', datasets={'stamped': child_db},
                          attributes={'tag': 't',
                                      'data_type': 'TimestampedRecord',
                                      'namespace': CORE_NAMESPACE,
                                      'object_id': 'rid'})
        record = _construct(parent_spec, [child_def], gb)
        self.assertEqual(record.stamped.data,
                         [datetime(2024, 3, 15), datetime(2024, 4, 1, 12, 0)])
    ...
    def test_inc_site_dtype_override(self):
        ...
        resolved_spec, and the construct path honors that subspec's dtype on read.
        """
        child_def = DatasetSpec(doc='generic', data_type_def='StampedThing')
        col_subspec = DatasetSpec(doc='date column', data_type_inc='StampedThing',
                                  name='stamped', dtype='isodatetime')
        parent_spec = GroupSpec(doc='record', data_type_def='TimestampedRecord',
                                attributes=[AttributeSpec('tag', 'a tag', 'text')],
                                datasets=[col_subspec])

        child_db = DatasetBuilder('stamped', ['2020-01-01', '2021-06-15'],
                                  attributes={'data_type': 'StampedThing',
                                              'namespace': CORE_NAMESPACE,
                                              'object_id': 'sid'})
        gb = GroupBuilder('r', datasets={'stamped': child_db},
    ...
    def test_non_datetime_dtype_unchanged(self):
        ...
        child_def = DatasetSpec(doc='stamped', data_type_def='StampedThing', dtype='int',
                                shape=(None,))
        ...
        ints = [1, 2, 3]
        child_db = DatasetBuilder('stamped', ints,
                                  attributes={'data_type': 'StampedThing',
                                              'namespace': CORE_NAMESPACE,
                                              'object_id': 'sid'})
        gb = GroupBuilder('r', datasets={'stamped': child_db},
