"""Tests for matched_spec wiring in ObjectMapper.__get_subspec_values matcher.

When __get_subspec_values pairs a sub-builder to a subspec from the parent's
spec, it records that subspec on the sub-builder via builder.matched_spec.
...
class TestMatchedSpecOnRead(TestCase):
    """The read-path matcher sets builder.matched_spec for each subspec kind."""
    ...
    def test_named_typed_dataset_child(self):
        ...
        self.assertIs(child_db.matched_spec, named_subspec)
    ...
    def test_typed_group_child(self):
        ...
        self._construct_with(parent_spec, [child_def], parent_gb)
        self.assertIs(child_gb.matched_spec, group_subspec)
    ...
    def test_unnamed_typed_dataset_children(self):
        ...
        self._construct_with(parent_spec, [child_def], parent_gb)
        self.assertIs(c1.matched_spec, unnamed_subspec)
        self.assertIs(c2.matched_spec, unnamed_subspec)

    def test_named_structural_subspec_recursive(self):
        """Named subspec with no data_type triggers recursion; the structural builder gets matched_spec."""
        ...
        self.assertIs(nested_gb.matched_spec, nested_subspec)

    def test_link_target_matched_spec_not_set_by_link_match(self):
        """A LinkBuilder target is not assigned matched_spec by the link-matching path.

        The link matcher constructs the target builder but does not record a matched_spec
        ...
        self._construct_with(parent_spec, [child_def], parent_gb)
        self.assertIsNone(target_gb.matched_spec)

    def test_write_once_preserved_across_matchers(self):
        """If a builder somehow already has matched_spec set, the matcher does not overwrite it."""
        child_def = DatasetSpec(doc='child', data_type_def='Child', dtype='int')
        ...
        child_db = DatasetBuilder('named', [1, 2],
                                  attributes={'data_type': 'Child', 'namespace': CORE_NAMESPACE,
                                              'object_id': 'cid'})
        child_db.matched_spec = sentinel  # pre-set
        parent_gb = GroupBuilder('p', datasets={'named': child_db},
                                 attributes={'attr1': 'v', 'data_type': 'Parent',
                                             'namespace': CORE_NAMESPACE, 'object_id': 'pid'})

        self._construct_with(parent_spec, [child_def], parent_gb)
        self.assertIs(child_db.matched_spec, sentinel)
    ...
    def test_dynamic_table_column_dtype_override(self):
        ...
        After construct, the column builder's matched_spec is the parent's DatasetSpec for that
        ...
        self.assertIs(col_db.matched_spec, col_subspec)
        self.assertEqual(col_db.matched_spec.dtype, 'isodatetime')
