...
from hdmf.build import CustomClassGenerator, TypeMap
from hdmf.build.classgenerator import ClassGenerator, MCIClassGenerator
from hdmf.container import AbstractContainer, Container, Data, MultiContainerInterface
...
class TestDynamicContainer(TestCase):
    ...
    def test_multi_container_spec(self):
        multi_spec = GroupSpec(
            ...
            data_type_def="Multi",
            groups=[GroupSpec(data_type_inc=self.bar_spec, doc="test multi", quantity="*")],
            attributes=[AttributeSpec(name="attr3", doc="a float attribute", dtype="float")],
        )
    ...
    def test_multi_container_spec_zero_or_more(self):
        ...
        self.spec_catalog.register_spec(multi_spec, "extension.yaml")
        Multi = self.type_map.get_dt_container_cls("Multi", CORE_NAMESPACE)
        multi = Multi(name="my_multi", attr3=5.0)
    ...
    def test_multi_container_spec_one_or_more_missing(self):
        ...
        with self.assertRaisesWith(
            TypeError,
            "MCIClassGenerator.set_init.<locals>.__init__: missing argument 'bars'",
        ):
            Multi(name="my_multi", attr3=5.0)
...
class TestGetClassSeparateNamespace(TestCase):
    ...
    def _build_separate_namespaces(self):
        ...
        create_load_namespace_yaml(
            ...
            specs=[baz_spec],
            output_dir=self.test_dir,
            incl_types={CORE_NAMESPACE: ["Bar"], "ndx-qux": ["Qux"]},
            type_map=self.type_map,
        )
...
class TestBaseProcessFieldSpec(TestCase):
    ...
    def test_update_docval(self):
        ...
        spec = GroupSpec(
            doc="A test group specification with a data type",
            data_type_def="Baz",
            groups=[GroupSpec(doc="a group", data_type_inc="EmptyBar", quantity="?")],
            datasets=[
                DatasetSpec(
                    doc="a dataset",
                    dtype="int",
    ...
    def test_post_process_default_name(self):
        ...
        expected = [
            {"name": "name", "type": str, "doc": "name", "default": "MyBaz"},
            {
                "name": "attr1",
                "type": ("array_data", "data"),
                "doc": "a string attribute",
...
class TestMCIProcessFieldSpec(TestCase):
    ...
    def test_post_process(self):
        multi_spec = GroupSpec(
            doc="A test extension that contains a multi",
            data_type_def="Multi",
            groups=[GroupSpec(data_type_inc="EmptyBar", doc="test multi", quantity="*")],
        )
        classdict = dict(
            __clsconf__=[
                dict(
