...
from hdmf.spec import DatasetSpec, DtypeSpec, RefSpec
...
class TestConvertDtype(TestCase):
    def test_value_none(self):
        ...
        spec = DatasetSpec(
            "an example dataset",
            RefSpec(reftype="object", target_type="int"),
            name="data",
        )
    ...
    def _test_change_basetype_helper(self, spec_type, value_types, exp_type):
        ...
        for dtype in value_types:
            ...
            with self.subTest(dtype=dtype):
                ...
                g = np.dtype(self._get_type(dtype))
                msg = (
                    "Spec 'data': Value with data type %s is being converted to data type %s (min specification: %s)."
    ...
    def test_compound_type(self):
        ...
        spec_type = [
            DtypeSpec("an int field", "f1", "int"),
            DtypeSpec("a float field", "f2", "float"),
        ]
        spec = DatasetSpec("an example dataset", spec_type, name="data")
        value = ["a", 1, 2.2]
