...
from hdmf.data_utils import DataChunk, DataChunkIterator
...
class DataChunkIteratorTests(TestCase):
    ...
    def test_none_iter(self):
        """Test that DataChunkIterator __init__ sets defaults and all chunks and recommended shapes are None."""
    ...
    def test_multidimensional_list_first_axis(self):
        """Test DataChunkIterator with multidimensional list data, no buffering, and iterating on the first dim."""
    ...
    def test_multidimensional_list_middle_axis(self):
        ...
        a = np.arange(30).reshape(5, 2, 3).tolist()
        warn_msg = (
            "Iterating over an axis other than the first dimension of list or tuple"
            " data involves converting the data object to a numpy ndarray, which may"
            " incur a computational cost."
        )
