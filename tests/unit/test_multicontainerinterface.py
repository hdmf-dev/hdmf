...
class TestNoClsConf(TestCase):
    ...
    def test_init_no_cls_conf(self):
        ...
        msg = (
            "MultiContainerInterface subclass Bar is missing __clsconf__ attribute."
            " Please check that the class is properly defined."
        )
        with self.assertRaisesWith(TypeError, msg):
            Bar(name="a")
...
class TestBadClsConf(TestCase):
    ...
    def test_create_multiple_types(self):
        ...
        msg = (
            "Cannot specify 'create' key in __clsconf__ for MultiContainerInterface"
            " subclass Bar when 'type' key is not a single type"
        )
        with self.assertRaisesWith(ValueError, msg):
    ...
    def test_create_multiple_types_multi(self):
        ...
        msg = (
            "Cannot specify 'create' key in __clsconf__ for MultiContainerInterface"
            " subclass Bar when 'type' key is not a single type at index 1"
        )
        with self.assertRaisesWith(ValueError, msg):
