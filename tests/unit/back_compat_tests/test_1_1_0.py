...
from ..helpers.utils import Foo, FooBucket, get_foo_buildmanager
...
class Test1_1_0(TestCase):
    ...
    def test_read_1_0_5(self):
        ...
        with HDF5IO(self.path_1_0_5, manager=self.manager, mode="r") as io:
            ...
            self.assertListEqual(
                read_foofile.buckets["test_bucket"].foos["foo1"].my_data[:].tolist(),
                [0, 1, 2, 3, 4],
            )
            self.assertListEqual(
                read_foofile.buckets["test_bucket"].foos["foo2"].my_data[:].tolist(),
                [5, 6, 7, 8, 9],
            )
    ...
    def test_append_1_0_5(self):
        ...
        with HDF5IO(self.path_1_0_5, manager=self.manager, mode="r") as io:
            read_foofile = io.read()
            self.assertListEqual(
                read_foofile.buckets["foobucket2"].foos["foo3"].my_data[:].tolist(),
                foo.my_data,
            )
