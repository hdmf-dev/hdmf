...
from hdmf.spec import AttributeSpec, DatasetSpec, GroupSpec, LinkSpec
...
class GroupSpecTests(TestCase):
    ...
    def test_set_parent_exists(self):
        GroupSpec("A test group", name="root_constructor", groups=self.subgroups)
    ...
    def test_set_link(self):
        ...
        link = LinkSpec(doc="A test link", target_type="LinkTarget", name="link_name")
        group.set_link(link)
    ...
    def test_add_link(self):
        group = GroupSpec(doc="A test group", name="root")
        group.add_link("A test link", "LinkTarget", name="link_name")
    ...
    def test_type_extension(self):
        ...
        dset1_attributes_ext = [
            AttributeSpec(
                "dset1_extra_attribute",
                "an extra attribute for the first dataset",
                "text",
            )
        ]
        ext_datasets = [
        ...
        ext_attributes = [
            AttributeSpec(
                "ext_extra_attribute",
                "an extra attribute for the group",
                "text",
            ),
        ]
        ext = GroupSpec(
    ...
    def test_path_complicated(self):
        ...
        dataset = DatasetSpec("my first dataset", "int", name="dataset1", attributes=[attribute])
        subgroup = GroupSpec("A subgroup", name="subgroup1", datasets=[dataset])
    ...
    def test_path_no_name(self):
        ...
        subgroup = GroupSpec("A subgroup", data_type_def="GroupType", datasets=[dataset])
        _ = GroupSpec("A test group", name="root", groups=[subgroup])
...
class TestNotAllowedConfig(TestCase):
    def test_no_name_no_def_no_inc(self):
        msg = (
            "Cannot create Group or Dataset spec with no name without specifying"
            " 'data_type_def' and/or 'data_type_inc'."
        )
...
class TestResolveAttrs(TestCase):
    ...
    def test_is_inherited_spec(self):
        ...
        self.assertTrue(
            self.inc_group_spec.is_inherited_spec(AttributeSpec("attribute1", "my first attribute", "text"))
        )
    ...
    def test_is_overridden_spec(self):
        ...
        self.assertFalse(
            self.inc_group_spec.is_overridden_spec(AttributeSpec("attribute1", "my first attribute", "text"))
        )
